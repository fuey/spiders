...
def parse_detail(response):
    ...
    item["type"] = type
    item["nation"] = response.xpath("//div[@id='titleDetails']/div[@class='txt-block'][2]/a/text()").extract()[0]
    item["language"] = " | ".join(response.xpath("//div[@id='titleDetails']/div[@class='txt-block'][3]/a/text()").extract()[0])
    ...
    item["data_source"] = 21 #imdb_movie_top250
    return item
    #item["summary_url"] = ""+response.xpath("//a[@class='slate_button prevent-ad-overlay video-modal']/@href").extract()[0]
...
class ImdbMovieTop250Spider(scrapy.Spider):
